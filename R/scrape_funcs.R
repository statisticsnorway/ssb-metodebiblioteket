...
get_repo_urls <- function(repo, github_token){
  ...
  response <- GET(url, add_headers(Authorization = paste0("Bearer ", github_token)))
...
traverse_directory <- function(contents) {
  ...
  for (item in contents) {
    if (item[["type"]] == "file") {
      ...
      nested_url <- item[["url"]]
      nested_response <- GET(nested_url, add_headers(Authorization = paste0("Bearer ", github_token)))
...
check_func <- function(func, url_data, github_token, type = "R"){
  ...
  for (i in 1:nrow(urls)){
    print(i)
    response <- GET(urls$url[i], add_headers(Authorization = paste0("Bearer ", github_token)))
